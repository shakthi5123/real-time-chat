...
export default function ChatPage() {
  ...
  return (
    <main className="flex flex-col sm:flex-row h-screen bg-gray-100">
      {/* LEFT SIDEBAR */}
      <aside className="w-full sm:w-1/3 md:w-1/4 bg-white border-r border-gray-200 shadow-md p-4 flex-shrink-0">
        <h2 className="text-xl font-semibold mb-3">Chats</h2>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <button
            onClick={() => signOut({ callbackUrl: "/login" })}
            className="ml-auto bg-red-500 text-white text-xs px-3 py-1 rounded-lg hover:bg-red-600"
          >
            Logout
          </button>
        </div>
        ...
        <ul className="space-y-2">
          {["Shakthi", "Muthuraj", "Vignesh", "Mukesh"].map((name) => (
            <li
              key={name}
              className="p-3 bg-gray-100 hover:bg-gray-200 rounded-lg cursor-pointer text-sm"
            >
              {name}
            </li>
          ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
          {messages.map((m, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`px-4 py-2 rounded-xl shadow max-w-[80%] text-sm 
                    ${
                      isMe
                        ? "bg-blue-500 text-white rounded-br-none"
